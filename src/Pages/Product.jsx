                            ...
                            <div className="product-description pb-3">
                                <div className="flas-sale-panel bg-white mb-3 p-3">
                                    <div className="container">
                                        ...
                                        {/* add to cart */}
                                        {product.status ? isAuthenticated ? (
                                            <div className="d-flex justify-content-start align-items-center">
                                                {inCart(product.id) ? (
                                                    <button onClick={() => removeFromCart(product.id)} className="btn btn-sm btn-danger">Remove from Cart</button>
                                                ) : (
                                                    <button onClick={() => addToCart(product.id)} className="btn btn-sm btn-warning">Add to Cart</button>
                                                )}
                                            </div>
                                        ) : (
                                            <div className="d-flex justify-content-start align-items-center">
                                                <button onClick={() => navigate('/login')} className="btn btn-sm btn-danger">Add to Cart</button>
                                            </div>
                                        ) : (
